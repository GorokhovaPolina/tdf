 ...
 */
    void CalculateHessian(cv::Mat image, float sigma_d, cv::Mat Hxx, cv::Mat Hxy, cv::Mat Hyy){
        ...
        cv::Mat dx_dy = dx.mul(dy);

        cv::GaussianBlur(dxSquared, Hxx, cv::Size(0, 0), sigma_d);
        cv::GaussianBlur(dx_dy, Hxy, cv::Size(0, 0), sigma_d);
        cv::GaussianBlur(dySquared, Hyy, cv::Size(0, 0), sigma_d);
    }
